	...
	public static class ParserBuilder
	{
		...
		public static bool GenerateLibrary(GrammarType type, string text, string @namespace, string path, List<Message> messages)
		{
			...
			if (messages.Count(m => m.Type == MessageType.Error) == 0)
			{
				...
				if (messages.Count(m => m.Type == MessageType.Error) == 0)
				{
					BuildLexer(builtGrammar, "CurrentLexer", messages);
					File.WriteAllText("Grammar.cs", GetGrammarProviderText(builtGrammar, @namespace));
					File.WriteAllText("Parser.cs", GetParserProviderText(@namespace));
					...
					var compilationResult = codeProvider.CompileAssemblyFromSource(compilerParams, "CurrentLexer.cs", "Grammar.cs", "Parser.cs");
...
namespace " + @namespace + @"
{
	public static class ParserProvider
	{
		public static BaseParser GetParser()
		{
			...
			{
				case GrammarType.LL:
					parser = new Parsing.LL.Parser(grammar,
						new AntlrLexerAdapter(
							(Antlr4.Runtime.ICharStream stream) => (Antlr4.Runtime.Lexer)Activator.CreateInstance(lexerType, stream)
						)
					);
					break;
				case GrammarType.LR:
					parser = new Parsing.LR.Parser(grammar,
						new AntlrLexerAdapter(
							(Antlr4.Runtime.ICharStream stream) => (Antlr4.Runtime.Lexer)Activator.CreateInstance(lexerType, stream)
						)
					);
